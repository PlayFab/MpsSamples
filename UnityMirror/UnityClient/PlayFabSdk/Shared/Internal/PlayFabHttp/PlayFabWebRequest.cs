...
namespace PlayFab.Internal
{
    public class PlayFabWebRequest : IPlayFabTransportPlugin
    {
        ...
        {
            // This needs to be improved to use a decent thread-pool, but it can be improved invisibly later
            var newThread = new Thread(() => SimpleHttpsWorker("PUT", fullUrl, payload, (result) => { successCallback(result); }, errorCallback));
            newThread.Start();
        }
        ...
        {
            // This needs to be improved to use a decent thread-pool, but it can be improved invisibly later
            var newThread = new Thread(() => SimpleHttpsWorker("POST", fullUrl, payload, (result) => { successCallback(result); }, errorCallback));
            newThread.Start();
        }
